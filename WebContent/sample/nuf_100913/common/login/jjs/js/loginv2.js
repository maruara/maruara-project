...
var fc_isWin = (navigator.appVersion.toLowerCase().indexOf("win") != -1) ? true
		: false;
...
function showFlash(_swfURL_, _flashID_, _width_, _height_, _wmode_,
		_flashVars_, _bgColor_, _fullscreen_) {
	...
	if (fc_isIE && fc_isWin && !fc_isOpera) {
		_object_ = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0" width="'
				+ _width_
				+ '" height="'
				+ _height_
				+ '" id="'
...
function savedID(objs) {
	if (objs.checked == true) {
		$('keep_uid1').checked = true;
		$('keep_uid2').checked = true;
		$('keep_uid3').checked = true;
		$('saveID').value = "1";
	} else {
		$('keep_uid1').checked = false;
		$('keep_uid2').checked = false;
		$('keep_uid3').checked = false;
		$('saveID').value = "0";
	}
}
...
function capslockevt(e) {
	...
	if ((myKeyCode >= 65 && myKeyCode <= 90) && !myShiftKey) {
		showErrorDiv(1);
		setTimeout("showErrorDiv(-1)", 3000);
...
function delayFocus(objnm) {
	try {
		setTimeout("$('" + objnm + "').focus()", 200)
	} catch (e) {
		try {
			setTimeout("$('" + objnm + "').focus()", 500)
		} catch (e) {
		}
	}
}
...
/* get key functions */
var getkeyurl = 'http://static.nid.naver.com/enclogin/keys.nhn';
var curtimecheck = 0;
var ajaxavail = -1;
var keystr = null;
var keys = null;
...
function idset_focus_run() {
	try {
		...
		// alert("code:" + e.number +":" + userid + ",:" + e.description)
		flashretry++;
		if (flashretry < 5) {
...
function onActiveXInst() {
	...
	$('level3_frame').src = "/loginv2/waitInst.nhn";
}
function needDelay() {
...
function useLevel(levelno) {
	...
	if (chglevel == 3) {
		$('level3_frame').src = "/loginv2/activex.nhn";
	} else if (chglevel == 2) {
...
function slideinit(levelno) {
	if ((navigator.userAgent.indexOf("Opera") != -1)||(navigator.userAgent.indexOf("Safari") != -1)||navigator.platform.indexOf("Mac") != -1 ||navigator.platform.indexOf("Linux") != -1) {
		levelno = 1;
	}
...
function clear3rdLevel() {
	if ($('level3_frame').src != "/loginv2/blank.html") {
		$('level3_frame').src = "/loginv2/blank.html";
	}
}
	...
	if (!onError) {
		if ((flags == '1' || flags == 1)
			...
			if (initEnc == 2) {
				...
				$('login_wrap').className = 'step1';
				$('slide_btn1').style.left = '22px';
			} else if (initEnc == 3) {
				$('login_wrap').className = 'step1';
				$('slide_btn1').style.left = '42px';
			}
			if (navigator.appVersion.indexOf("MSIE 8")) {
				$('flashlogin').style.visibility = 'hidden';
			}
			// alert('slide_btn'+initEnc);
			$('level_img1').src = 'http://static.naver.com/green/v2/img/h_security' + initEnc + '.gif';
		} else {
			$('slide_btn1').style.left = '2px';
			if (swapLevel == 2) {
			...
			$('level_img1').src = 'http://static.naver.com/green/v2/img/h_security1.gif';
			swapLevel = -1;
		}
	}
}
...
function setIdTextField() {
	...
	idHeight = "20";
	idMaxChars = "12";
	idTextDefault = "http://static.naver.com/common/login/bg_login_id_main_3.gif";
	idTextFont = "";
	idBoldTag = ""; // true
	idTextColor = "444444";
	idTextSize = "12";
...
function setPwTextField() {
	...
	pwHeight = "20";
	pwMaxChars = "16";
	pwTextDefault = "http://static.naver.com/common/login/bg_login_pw_main_3.gif";
	pwTextFont = "";
	pwBoldTag = ""; // true
	pwTextColor = "444444";
	pwTextSize = "12";
